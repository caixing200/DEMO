...
Page({
  ...
  search: function (e) {
    ...
    if (formData.empId != '') {
      wx.redirectTo({//跳转人员生产合格率
        url: '../../peppassedrate/pepperform?empId=' + formData.empId + '&&beginDate=' + this.data.startDate + '&&endDate=' + this.data.endDate,
      })
    } else {
      wx.redirectTo({//跳转部门生产合格率
        url: '../../passedrate/passedrate?dept=' + JSON.stringify(formData.deptId) + '&&start=' + this.data.startDate + '&&end=' + this.data.endDate,
      })
    }
    return false;
  },
