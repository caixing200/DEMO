...
Page({
  ...
  onLoad: function (options) {
    if (options && options.endDate) {
      var dept = { "name": options.deptName, "id": options.deptId }
      this.setData({
  ...
  search: function (e) {
    ...
    if (this.data.deptId == '' && this.data.empId == '') {
      wx.showModal({
        showCancel: false,
        content: '请选择人员或部门',
      })
    ...
    if (formData.empId != '') {
      wx.redirectTo({//跳转人员生产效率
        url: '../../pepefficiencyrate/pepperform?empId=' + formData.empId + '&&beginDate=' + this.data.startDate + '&&endDate=' + this.data.endDate,
      })
    } else {
      wx.redirectTo({//跳转部门生产效率
        url: '../../efficiency/efficiency?dept=' + JSON.stringify(formData.deptId) + '&&start=' + this.data.startDate + '&&end=' + this.data.endDate,
      })
    }
    return false;
  },
